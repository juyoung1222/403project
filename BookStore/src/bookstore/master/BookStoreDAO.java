...
public class BookStoreDAO 
{
	...
	public int managerCheck(String id, String passwd) throws Exception {
		...
		try {
			...
			if(rs.next()) { //id에 해당하는 자료가 있다면
				...
				dbpasswd = rs.getNString("managerPasswd");
	...
	//----------------------------------------------------------------------------------------------
	//book_id에 해당하느 정보를 추출한다.
	//----------------------------------------------------------------------------------------------
	public BookDTO getBook(int book_id) throws Exception{
		Connection			conn		= null;
		PreparedStatement	pstmt		= null;
		ResultSet			rs			= null;
		String				sql		= "";
	...
	//----------------------------------------------------------------------------------------------
		...
		//----------------------------------------------------------------------------------------------
				public int getBookIdCount(int book_id) throws Exception{
					...
					try {
						...
						if(rs.next()) {
							//rtnCount= rs.getInt(book_count); 이렇게 직접 써도 된다.
							rtnCount= rs.getInt(1);
				...
				}//end-public int getBookIdCount(int book_id)
