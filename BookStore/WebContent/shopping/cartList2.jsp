...
<%@ page import="java.util.List" %><%//여려건의 데이터를 사용 %>
<%@ page import="java.text.NumberFormat" %><%//숫자를 형식에 맞춰 %>
<%@ page import="com.oreilly.servlet.MultipartRequest" %><%//사진가져오기 %>

<%
//로그인한 사용자가 장바구니에 담은 정보를 보여주자!
...
//사용자 아이디를 알아야 사용자 아이디에 해당하는 정보를 가져올수 있으므로 먼저 세션에서 사용자 정보를 추출

String buyer = (String)session.getAttribute("id");
String book_kind = request.getParameter("book_kind");
//작업에 필요한 변수들을 정의한다.
int number = 0;//화면에 데이터건수를 일련번호로 보여주기 위해서
...
//realFolder	= "http://localhost:8099/BookStore/imageFile";

//사용자 아이디에 해당하는 정보들을 가져온다.
...
if(count<1) {//보여줄 장바구니의 내용이 없으면%>
	...
	<input class="btn btn-info btn-xs" type="button" value="책방둘러보기" onclick="javascript:window.location='bookList.jsp?book_kind=<%=book_kind %>'">
<%}else{ 
	...
	<form name="cartForm" method="post" action="updateCartPro.jsp">
		<table class="table table-bordered table-striped table-hover">
			...
			<tr>
				...
				<td align=right><%=NumberFormat.getInstance().format(cartList.getBuy_price())%></td>
				<td><input type="text" style="text-align: right" name="buy_count" size="4" value="<%=cartList.getBuy_count() %>"disabled>
				<%String url="updateCartForm.jsp?cart_id=" + cartList.getCart_id() + "&book_kind=" + book_kind + "&buy_count=" + cartList.getBuy_count() + "&book_id=" + 
				cartList.getBook_id();%>
					<input class="btn btn-warning btn-xs" type="button" value="변경"
					onclick="javascript:window.location='<%=url %>'">
				<!--  <td align=right><%=cartList.getBuy_count() %></td>-->
				<!--  <td align=right><%=NumberFormat.getInstance().format(cartList.getBuy_price() * cartList.getBuy_count()) %></td>-->
				<td align="right">
					...
					<input class="btn btn-danger btn-xs" type="button" value="삭제" onclick="javascript:window.location='cartListDel.jsp?cart_id=<%=cartList.getCart_id() %>&book_kind
					 =<%=book_kind %>'">
				</td>
			<%} %>
			<tr class="danger">
				<td colspan="5" align="right">
					<h4>총금액 : <%=NumberFormat.getInstance().format(total) %>&nbsp;원&nbsp;</h4>
			</tr>
			<tr>
