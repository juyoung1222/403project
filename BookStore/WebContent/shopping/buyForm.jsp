...
<%//구매 확정전 화면을 구성한다.
  //확정된 장바구니 목록의 수량, 구매예정금액을 보여준다.
  //사용자에 대한 정보를 보여주고 배송지 정보는 수정할 수 있게 한다.
  
  //세션아이디가 있는지(정상적으로 로그인하고 입장하였는지) 검사한다.
...
<div class="container">
	...
	<table class="table table=striped table-hover">
		...
		for(int i =0;i<cartLists.size();i++){
			cartList = cartLists.get(i);//cartLists에서 한건을 추출한 후에 자룔르 출력한다.%>
			
			<tr>
				<td align=right><%=++number %></td>
				<td>
					<img src="<%=realFolder %>/<%=cartList.getBook_image() %>" width="30" height="50"/>&nbsp;&nbsp;<%=cartList.getBook_title() %>
				</td>
				<td align=right valign=bottom>
					<%=NumberFormat.getInstance().format(cartList.getBuy_price()) %>원
				</td>
				<td align=right>
					<%=NumberFormat.getInstance().format(cartList.getBuy_count()) %>
				</td>
				<td align=right>
		...
		<tr class="danger">
			<td colspan="5" align="right">
				<b><font size="+1">총 구매금액 : <%=NumberFormat.getInstance().format(totalAmount) %> 원</font></b>
			</td>
		</tr>
</table>
...
<form class="form-horizontal" method="post" name="buyInput" action="buyPro.jsp">
	...
	<div class="form-group">
		...
		<div class="col-sm-3">
			<input type="text" class="form-control" id="tel" name="tel" value="<%=member.getTel() %>"/>
		</div>
	</div>
	<div class="form-group">
	...
	<div class="form-group">
		...
		<div class="col-sm-3">
			<select class="form-control" id="account" name="account">
				<%//게좌번호,은행명,계좌소유주를 하나로 합쳐거 option항목에 보이게 한다.
	...
	<div class="form-group">
		<div class="col-sm-offset-2 col-sm-4">
			...
			<input class="btn btn-danger btn-sm" type="submit" value="취소" onclick="javascript:window.location='cartList.jsp?book_kind=all'"/>&nbsp;&nbsp;
		</div>
