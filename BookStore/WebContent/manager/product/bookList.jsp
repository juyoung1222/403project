...
%>
	<table class="table table-bordered table-striped table-hover">
		...
		<tr>
			...
			<td align=center>
				...
				<%=book.getBook_id()%>&book_kind=<%=URLEncoder.encode(book.getBook_kind(),"UTF-8")%>">수정</a><!-- get방식일때 한글로 넘겨줘야 할때 사용. -->
			</td>
			<td align=center>
	...
	if(totalCount > 0) {
		...
		//이전 페이지로 갈수 있도록함.
		//startPage가 pageBlock보다 큰 경우에는 [이전]버튼을 보여준다.
		//[이전]버튼을 누르면 현재 보고있는 화면의 startPage에서 pageBlock만큼 뺀 같이 다음화면의 첫 페이지가 되도록 함.
		...
		<%}
		//다음버튼이 나타나는 경우
		//화면에 보여줄 총 페이지 건수보다 endPage가 작은 경우만 [다음]버튼을 나타나게함.
